    ...
    def draw(self, context):
        ...
        if op.poseCOL:
            ...
            if op.pidx > 0:
                ...
                row.label(text="", icon='MOUSE_MMB')
                row.label(text=f"Preview Alpha: {dynamic_format(op.empty.M3.group_pose_alpha, 0)}")
    ...
    def draw_HUD(self, context):
        if self.area == context.area:
            ...
            if self.pidx > 0:
                ...
                draw_label(context, title=dynamic_format(self.empty.M3.group_pose_alpha, 0), coords=Vector((self.HUD_x + dims[0] + max_dim + alpha_dims[0], self.HUD_y)), offset=self.offset, center=False, color=white, alpha=1)
...
def draw_setup_group_gizmos_status(op):
    def draw(self, context):
        ...
        row.label(text="", icon='MOUSE_MMB')
        row.label(text=f"Adjust Size: {dynamic_format(op.empty.M3.group_gizmo_size, decimal_offset=decimal_offset)}")
        ...
        row.label(text="", icon='EVENT_X')
        row.label(text=f"X Axis Gizmo: {op.empty.M3.show_group_x_rotation}")

        row.separator(factor=1)

        row.label(text="", icon='EVENT_Y')
        row.label(text=f"Y Axis Gizmo: {op.empty.M3.show_group_y_rotation}")

        row.separator(factor=1)

        row.label(text="", icon='EVENT_Z')
        row.label(text=f"Z Axis Gizmo: {op.empty.M3.show_group_z_rotation}")
        ...
        row.label(text="", icon='EVENT_S')
        row.label(text=f"Show Gizmo{'s' if len(op.axes) > 1 else''}: {op.empty.M3.show_group_gizmo}")
...
class SetupGroupGizmos(bpy.types.Operator):
    ...
    def draw_HUD(self, context):
        if self.area == context.area:
            ...
            dims2 = draw_label(context, title=f"{dynamic_format(self.empty.M3.group_gizmo_size, decimal_offset=decimal_offset)} ", coords=Vector((self.HUD_x + dims[0], self.HUD_y)), offset=self.offset, center=False, color=white, alpha=1)
            ...
            if context.scene.M3.group_gizmo_size != 1:
                ...
                draw_label(context, title=f" Global: {dynamic_format(context.scene.M3.group_gizmo_size)}", coords=Vector((self.HUD_x + dims[0] + dims2[0] + dims3[0] + dims4[0], self.HUD_y)), offset=self.offset, center=False, color=white, alpha=0.25)
